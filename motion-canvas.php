 ...
 */
function create_block_motion_canvas_block_init() {
	wp_register_block_types_from_metadata_collection( __DIR__ . '/build', __DIR__ . '/build/blocks-manifest.php' );
}
add_action( 'init', 'create_block_motion_canvas_block_init' );
...
function motion_canvas_allowed_mime_types( $mimes ) {
...
add_filter( 'upload_mimes', 'motion_canvas_allowed_mime_types' );

// Fix MIME type check for these file types
function motion_canvas_fix_mime_check( $data, $file, $filename, $mimes ) {
...
add_filter( 'wp_check_filetype_and_ext', 'motion_canvas_fix_mime_check', 10, 4 );
/**
 * Enqueue the Motion Layout Builder panel in the block editor.
 */
function motion_canvas_enqueue_layout_builder() {
...
add_action( 'enqueue_block_editor_assets', 'motion_canvas_enqueue_layout_builder' );
/**
 ...
 */
function motion_canvas_enqueue_reveal() {
...
add_action( 'wp_enqueue_scripts', 'motion_canvas_enqueue_reveal' );